...
Optionicons (CodeMod hook, gone in sm5)

DifficultyList (song change hooks)
...
local pane=function(pn)
	...
	return Def.ActorFrame {
		InitCommand=function(s)
			...
			if GAMESTATE:IsPlayerEnabled(pn) then InitPlayer(pn) s:playcommand("SlideIn") end
			s:ztestmode("writeonpass")
		end,
		SongChosenMessageCommand=function(s)
			if GAMESTATE:IsPlayerEnabled(pn) then
				s:playcommand("SlideOut")
			end
		end,
...
local function extrapane(pn)
	...
	return Def.ActorFrame {
		InitCommand=function(s)
			...
			if GAMESTATE:IsPlayerEnabled(pn) then s:playcommand("SlideIn") end
			s:ztestmode("writeonpass")
		end,
		SongChosenMessageCommand=function(s)
			if GAMESTATE:IsPlayerEnabled(pn) then
		...
		Def.Sprite {
			Texture="SelectMusic underlay/extra pane p"..PlayerIndex[pn],
			SetPlayerCommand=function(s) ApplyUIColor(s,pn) end,
		},
...
--Wheel mask
	Def.Quad {
		InitCommand=function(s)
			s:x(THEME:GetMetric("SelectMusic","MusicWheelX"))
			s:y(THEME:GetMetric("SelectMusic","MusicWheelY"))
			s:zoomto(352,24*(THEME:GetMetric(THEME:GetMetric("SelectMusic","MusicWheelType"),"NumWheelItems")-2))
			s:clearzbuffer(true)
